...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (scrollView.superview.tag == 2000 || scrollView.superview.tag == 2001) {
        return;
    }
...
//    NSLog(@"scrollView.contentOffset.y %f",scrollView.contentOffset.y);
    ...
    if (_end!=nil) {
        _end(scrollView.contentOffset.x);
    }
}

-(void)scrollViewWillDidScrollViewEnd:(scrollEndView)block{
    self.end = block;
}
